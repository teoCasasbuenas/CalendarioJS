var Calendario = /** @class */ (function () {
    function Calendario(fechaInicio, fechaFin, config) {
        this.defaultConfig = {
    ...
    Calendario.prototype.init = function () {
        ...
        this.monthHead = (function (days) {
            var weekdays = $(document.createElement('div')).addClass('calendar__weekdays');
            for (var day in days) {
    ...
    Calendario.prototype.DragEvents = function () {
        var that = this;
    ...
    Calendario.prototype.drawMonth = function (year, month) {
        ...
        monthWrapper = $(document.createElement('div')).addClass('calendar__monthwrapper calendar__month').attr({ 'data-yy-mm': year + '-' + (month < 9 ? '0' : '') + (month + 1) }), monthHeader = $(document.createElement('header')), monthLabel = $(document.createElement('h2')).addClass('calendar__monthlabel widht100').html(this.config.months[month] + ' ' + year), monthWeekdays = this.monthHead.clone(), monthBody = $(document.createElement('div')).addClass('calendar__monthbody').attr({}), weekRowWrapper = $(document.createElement('div')).addClass('calendar__weekrow'), i = 1;
        ...
        var currWeekrow = weekRowWrapper.clone(), weekDayControl = initWeekDay;
        while (initWeekDay > 0) {
            var emptyDate = $("<div class='inline-block prevmonth day emptyday'></div>");
            currWeekrow.addClass('font0').append(emptyDate);
            emptyDate.addClass((initWeekDay == 1 || initWeekDay == 6) ? 'hide' : '');
            initWeekDay--;
        }
        while (i <= daysInMonth) {
            var currDay = $("<div class='inline-block day" + (moment().get('date') == i && (moment().get('month') == month) ? ' today' : '') + "'><span class='calendar__daynumber'>" + i + "</span><div class='droppable-container'></div><a class='hide' href='#'>Más <i class='fa fa-plus'></i></a></div>").attr({ 'data-yy-mm-dd': year + "-" + (month < 9 ? '0' : '') + (month + 1) + "-" + (i < 10 ? '0' : '') + i });
            if (weekDayControl > 6) {
     ...
     * @param date
     */
    Calendario.prototype.addObject = function (obj, date) {
    ...
    Calendario.prototype.checkDroppableOverflow = function (el) {
        ...
        if ($(el).innerHeight() <= el.scrollHeight) {
            ...
            var elementsHeight_1 = 0, hiddenCount_1 = 0;
            $(el).find('> *').each(function () {
                //console.log('entra', this);
                $(this).removeClass('hide');
                elementsHeight_1 += $(this).outerHeight(true);
                if (elementsHeight_1 > $(el).innerHeight()) {
                    $(this).addClass('hide');
                    hiddenCount_1++;
                    $(el).closest('.day').find('a').removeClass('hide').html(hiddenCount_1 + " m\u00E1s");
                }
            });
        }
        else {
